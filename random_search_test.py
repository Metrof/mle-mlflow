...
metrics["logloss"] = log_loss(y_test, probas)

# Метрики CV (усредненные по результатам поиска)
...
input_example = X_test[:10] # Первые 10 объектов как просил тренажер

exp = mlflow.get_experiment_by_name(EXPERIMENT_NAME)
experiment_id = exp.experiment_id if exp else mlflow.create_experiment(EXPERIMENT_NAME)

with mlflow.start_run(run_name=RUN_NAME, experiment_id=experiment_id) as run:
    # Логируем лучшие параметры и метрики
    mlflow.log_params(best_params)
    mlflow.log_params({f"best_{k}": v for k, v in best_params.items()})
    mlflow.log_metrics(metrics)
    
    # Модель CatBoost (artifact_path='models' по заданию)
    ...
    # Объект поиска (artifact_path='cv' по заданию)
    ...
    print(f"Успех! Random Search Run ID: {run.info.run_id}")
